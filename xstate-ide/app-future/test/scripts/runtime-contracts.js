 ...
 *             and multi-step guard validation
 *
...
const INLINE_MACHINE = {
    ...
    states: {
        ...
        ask_path: {
            ...
            on: {
                PATH_A: { target: 'done_a' },
                PATH_B: { target: 'done_b' },
            },
        },
        done_a: {
...
const INLINE_SERVICES = {
    actions: {
        setName:       ({ event }) => ({ name: event.value }),
        setInputError: ({ event }) => ({ inputError: event.value }),
    },
    guards: {
...
const REALTOR_MACHINE_URL  = new URL('../fixtures/realtor-machine.json',  import.meta.url).href;
const REALTOR_SERVICES_URL = new URL('../fixtures/realtor-services.js',   import.meta.url).href;
...
const BASELINE_URL = './test/baselines/runtime-baseline.json';
