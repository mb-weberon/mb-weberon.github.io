...
const MACHINE_URL  = new URL('./realtor-machine.json', import.meta.url).href;
const SERVICES_URL = new URL('./realtor-services.js',  import.meta.url).href;
...
async function load_contracts() {
    ...
    if (totalIssues === 0) {
        console.log('✅ Load Contracts — all passed');
    } else {
        console.error(`❌ Load Contracts — ${totalIssues} issue(s)`);
    }
