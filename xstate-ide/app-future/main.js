...
import { render as preactUnmount } from 'preact';
...
async function boot() {
    ...
    function _showBlankSlate() {
        preactUnmount(null, chatMount);   // cleanly unmount Preact so __k is cleared
        chatUI = null;                    // next _activateFlow creates a fresh ChatUI
        chatMount.innerHTML = `
    ...
    function renderIDE(snap) {
        if (!config) return;
        const { stateId, context, userInput, trace } = snap;

        if (_lastStateId && _lastStateId !== stateId && userInput) {
            visitedEdges.add(`${_lastStateId}|${userInput}`);
        }
        _lastStateId = stateId;

        const traceRows = (trace?.steps ?? []).map((s, i) => {
        ...
        const traceHeader = trace
            ? `_trace: session=${trace.sessionId?.slice(0,8)}… flow=${trace.flowId}@${trace.flowVersion}\n${traceRows || '  (no steps yet)'}`
            : '_trace: null';
    ...
    // Strips action strings that were once required in machine.json to enable
    // session tracing (initTrace, record, recordValidationFailure, recordService).
    // These are now handled automatically by the Runtime's _onUpdate — machine.json
    // should not reference them. Called on every load; mutates config in-place.

    const RUNTIME_ACTIONS = new Set(['initTrace', 'record', 'recordValidationFailure', 'recordService']);

    function stripRuntimeActions(cfg) {
        let removed = 0;

        function filterActions(actions) {
            if (actions == null) return actions;
            const isSingle = !Array.isArray(actions);
            const list     = isSingle ? [actions] : actions;
            const filtered = list.filter(a => {
                if (typeof a === 'string' && RUNTIME_ACTIONS.has(a)) { removed++; return false; }
                return true;
            });
            if (isSingle) return filtered[0] ?? null;
            return filtered;
        }
        ...
        function stripBranches(branches) {
            ...
            list.forEach(t => { if (t) t.actions = filterActions(t.actions); });
        }

        cfg.entry = filterActions(cfg.entry);

        Object.values(cfg.states ?? {}).forEach(state => {
            state.entry = filterActions(state.entry);
            state.exit  = filterActions(state.exit);
            if (state.on) Object.values(state.on).forEach(t => stripBranches(t));
            stripBranches(state.always);
            if (state.invoke) {
                stripBranches(state.invoke.onDone);
                stripBranches(state.invoke.onError);
            }
        });
    ...
    function _activateFlow(newConfig, keepOverrides = false, prebuiltMachine = null) {
        ...
        // Strip runtime-internal action references that older machine.json files
        // may contain (initTrace, record, recordValidationFailure, recordService).
        // These are now handled automatically by the Runtime — machine.json must
        // not reference them. Mutates newConfig in-place before it becomes config.
        const removedCount = stripRuntimeActions(newConfig);
        if (removedCount > 0) {
            console.info(
                `%c🔧 Removed ${removedCount} legacy runtime action reference(s) from machine.json ` +
                `(initTrace / record / recordValidationFailure / recordService). ` +
                `These are now automatic — save the flow to persist the clean version.`,
                'color:#61afef'
            );
        }
        ...
        // For the first load (or after _showBlankSlate/_showRestorePrompt unmounted
        // Preact), chatUI is null → clear the static placeholder and create fresh.
        if (chatUI) {
...
window.addEventListener('load', () => {
    ...
    console.log(
        ...
        `                  now automatic — do not reference these in machine.json.\n\n` +
        `── Static Analysis ──\n` +
        `  window.analyzeFlow()                    — unreachable states, guard gaps, dead transitions\n` +
        `  window._analysisReport                  — last report object\n\n` +
        `── TypeScript Export ──\n` +
        `  window.exportToTypeScript()             — download {id}-xstate.zip (machine.ts + package.json + tsconfig.json)\n` +
